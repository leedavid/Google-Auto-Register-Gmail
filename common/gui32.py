...
class Cmygui32:
    ...
    def AutoRegistGmail(self):
        ...
        while _num > 0:
            ...
            if Gsql._success == True:
                ...
                print("注册成功！ 用时：", self.timeEnd - self.timeBegin)
                print("Emai: ", Gsql._gmail._email.lower() + '@gmail.com')
    ...
    def Regprepare(self):
        ...
        # 极光 VPN 
        while self.SearchImage('vpn_jg_connected', 'tmp', True) == True:  
        ...
        # 按下Gmail图标
        while self.ImgLeftClick("play_store_icon", "tmp") == False:
    ...
    def RegOneGmail(self, _Gsql):   
        ...
        while self.SearchImageAndInputName('gmail_input_name_1','tmp',
         _Gsql) == False: 
            ...
            if self.ImgLeftClick("play_store_icon", "tmp",False) == True:
                print('发现 play store 图标')

            # play store sign
            if self.ImgLeftClick("play_store_sign", "tmp",False) == True:
                print('发现 play store sign 图标')
        ...
        while True:
            ...
            if self.SearchImage('err_we_cannot_creat_accout', 'tmp', True) == True:
                print('暂时不能注册，按一下Next')
                # self.ImgLeftClick('err_not_next', 'tmp')
                _Gsql._success = False
                _Gsql._err = '暂时不能注册'  
                return

           
            if self.SearchImage('gmail_input_birth_1','tmp') == True:
                break
            else:               
                ...
                if self.SearchImageAndConfirmNotRobot('GetaVerification_flag','tmp',
                    _Gsql) == True  or self.SearchImageAndConfirmNotRobot('usa_qi','tmp',
                    _Gsql) == True  or self.SearchImageAndConfirmNotRobot('usa_qi2','tmp',
                    _Gsql) == True:
                    time.sleep(1.0)
                    #print('已点击 证明你不是机器人.')

                    print('输入 手机...sms')   # 可以 skip
                    if self.SearchImageAndInputSMS('enter_sms_code','tmp',
                    _Gsql) == True: 
                        time.sleep(1.0)
                        print('已输入手机sms')
                    else:
        ...
        while self.SearchImageAndInputEmail('gmail_input_email_1','tmp',
         _Gsql) == False: 
            print('waiting 输入email...')

            # 再输入年月日
            if self.SearchImageAndInputBirthDay('gmail_input_birth_1','tmp',
                _Gsql) == True: 
                print('waiting 输入基本信息OK')                

       
        while self.SearchImageAndInputPassword('gmail_input_password_1','tmp',
         _Gsql) == False: 
        ...
        time.sleep(1.0)
        #while self.SearchImage('add_play_stoer', tmp, True)   
                 
        # 等待进入谷歌商店
        while self.SearchImage('google_play_store','tmp',True) == False:

            if self.ImgLeftClick('google_service_accept','tmp') == True:
                print('接受谷歌备份协议')
            
            #if self.SearchImage('google_payinfo_no_thanks','tmp', True) == False:
            if self.SearchImage('google_no_thanks_no_sel','tmp', True) == True:
                self.WinLeftClick(self._hwndMain, self.imgx - 50, self.imgy)
                print('select No thans')  
                time.sleep(2.0)
                if self.ImgLeftClick('pay_continue','tmp', True) == True:
                    print('entering google play store ...')
                    time.sleep(3.0)  

            if self.ImgLeftClick('accept_enter_play','tmp', False):
                print('push accept ! ')

            print('waiting enter into play store')             
    ...
    def SearchImageAndConfirmNotRobot(self, _Tname, _Mname, _Gsql, isAgain = False):
        ...
        while True:
            ...
            # 1. 删除原来的手机号码 WM_CHAR
            if self.SearchImage('chinese_qi',_Mname) == True or self.SearchImage('usa_qi', 
            _Mname) == True or self.SearchImage('usa_qi2',_Mname) == True:
                pass
                #print("找到国棋")               
            else:
                print("找不到中国国棋")
            ...
            ph = '+86' + Gsql._gmail._phone

            self.send_input_hax(ph)
            time.sleep(1.5)  

            print('input phone: ', ph)
    ...
    def SearchImageAndInputEmail(self, _Tname, _Mname, _Gsql):        
        ...
        print('input email: ', _Gsql._gmail._email.lower() + '@gamil.com') 
